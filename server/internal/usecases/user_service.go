...
type (
	...
	UserService interface {
		...
		GetAllUsers(ctx context.Context) []*entities.UserEntity
	}

	// UserDBService
	UserDBService interface {
		Store(ctx context.Context, userEntity *entities.UserEntity) *http_utils.ErrorResponse
		Get(ctx context.Context, userID uint64) (*entities.UserEntity, *http_utils.ErrorResponse)
		GetAll(ctx context.Context) []*entities.UserEntity
	}
)
